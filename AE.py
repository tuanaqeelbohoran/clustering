...
#dataset =r'C:\Users\SG19\Desktop\images\trainingSampleMNIST'
#dataset =r'C:\Users\SG19\Desktop\AA'
print("[INFO] loading images...")
print("[INFO] loading",dataset)
#
#imagePaths = sorted(list((dataset)))
...
#KR.tensorflow_backend._get_available_gpus()




inputs = Input(shape=(WHD,))
h1 = Dense(32, activation='relu')(inputs)
h2 = Dense(16, activation='relu', activity_regularizer= l1(1e-5))(h1)
h3 = Dense(32, activation='relu')(h2)
outputs = Dense(WHD)(h3)

model = Model(inputs=inputs, outputs=outputs)
model.compile(optimizer='adam', loss='mse')

model.fit(X, X, batch_size=32, epochs=5,shuffle=False)

model.summary()
Layer_name  =  'dense_2'
cluster = model.get_layer(Layer_name).output
CL = Model(inputs=model.input, outputs=cluster, name='cluster_10')
...
###############################################################################
#transforming input image values to 2D using TSNE

sklearn_tsne = TSNE(learning_rate=100)
X_proj = sklearn_tsne.fit_transform(X_proj)


#X_proj = bhtsne(n_components=2).fit_transform(X_proj)
#X_proj.shape
################################################################################
#transforming input image values to 3D using TSNE
#X_proj3D = bhtsne(n_components=3).fit_transform(X_proj)
#X_proj3D.shape
sklearn_tsne = TSNE(n_components=3,learning_rate=100)
X_proj3D = sklearn_tsne.fit_transform(X_proj)
#sklearn_tsne = TSNE(n_components=3,learning_rate=100)
#X_proj3D = sklearn_tsne(n_components=3).fit_transform(X_proj)
###############################################################################


#predicting K-Means Values via AE
...
#
k_digits = KMeans(n_clusters=K,random_state=None)
y_pcapred=k_digits.fit_predict(X_proj3D,Y)
#predicting the cluster centroidspoints 2D
...
#with open("Collection.txt", "w") as text_file:
#    text_file.write("")

#for n in range(len(collection_pca_KMean)):
##    with open("Collection.txt", "a") as text_file:
##        text_file.write(str(collection_pca_KMean[n]))
##        text_file.write("\n")
#        worksheet.write(xlrow+int(n), xlcol+1, collection_pca_KMean[n])
...
#sample_silhouette_values = silhouette_samples(X_proj, y_pcapred)
...
##
##    ax.scatter(X_proj[:, 0][i], X_proj[:, 1][i], X_proj[:, 2][i], c=LABEL_COLOR_MAP[Y[i]],  marker= Marker[y_pcapred[i]])
##    i +=1
##
##i=0
##for i in range(len(X)):
##    ax.scatter(X_proj[:,0][i], X_proj[:,1][i], X_proj[:,2][i], c='None')
##    ax.text(X_proj[:,0][i], X_proj[:,1][i],X_proj[:,2][i],str(lbl_for_plot[Y[i]]),color=LABEL_COLOR_MAP[y_pcapred[i]],horizontalalignment='center',verticalalignment='center',size=12)
##    i +=1
##
##
##ax.scatter(centers1[:, 0], centers1[:, 1], centers1[:, 2], marker='x', c='#050505', s=1000)
##for i in range(K):
##    ax.text(centers1[:,0][i], centers1[:,1][i], centers1[:,2][i],cluster[i])
##collection_pca_KMean =dict(collection_pca_KMean)
##lgnd_lbl=[]
#
#
...
#ax.set_title('AE to K-Means', fontsize=10) 
...
#    
#plt.title('AE to K-Means', fontsize=10) 
...
##        lgnd_lbl.append(labels1)
##        i +=1
#plt.legend(loc='upper right',bbox_to_anchor=(1,1),handles=lgnd_lbl, fancybox=True)
##fig.savefig('C:/Users/SG19/Desktop/RisingStar_0001/PyScripts/plots/fig2.png', dpi=fig.dpi) 
#
#x=X_proj[:,0]
#y=X_proj[:,1]
#
